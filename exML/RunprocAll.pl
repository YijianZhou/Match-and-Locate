...
@ARGV == 7 || die"perl $0 event year month day dlenth template trace\n";
$event = $ARGV[0];
chomp($event);
...
foreach $_(@par){
        ...
        $L = "$evla/$evlo/$evdp/$evmg";
        $F = "$evla/$evlo/$evdp";
        print"$name $year/$month/$day\n";
        system("perl PROC_MatchLocate.pl $dir1/$name $dir2/$year$month$day $L $dir1/INPUT/$name $F");
        system("mv EventCase.out $name");
        system("mv $name $outfile/$name");
}
}
